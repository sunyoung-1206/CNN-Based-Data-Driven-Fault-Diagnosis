# 문제점 해결: 함수 매개변수 문제 수정 (데이터 증강 제거 버전)
...
# 경로 설정
IMAGESAVEURL_training = "/home/potato/박선영/CNN-Based-Data-Driven-Fault-Diagnosis/test2_generated_data/train_images"
IMAGESAVEURL_validation = "/home/potato/박선영/CNN-Based-Data-Driven-Fault-Diagnosis/test2_generated_data/test_images"

# Excel 파일 로드
training_wb = openpyxl.load_workbook('/home/potato/박선영/CNN-Based-Data-Driven-Fault-Diagnosis/test2_generated_data/trainingImageList.xlsx')
training_sheet = training_wb.active
validation_wb = openpyxl.load_workbook('/home/potato/박선영/CNN-Based-Data-Driven-Fault-Diagnosis/test2_generated_data/validationImageList.xlsx')
...
def load_data(sheet, img_dir, num_samples):
    ...
    for i in range(num_samples):
        try:
            ...
            if imagename is None or label_index < 0 or label_index >= 5:
                skipped += 1
                continue
...
def create_cnn_model():
    ...
    # 완전 연결 레이어 1
    x = tf.keras.layers.Dense(1536, activation='relu',
                             kernel_regularizer=regularizer)(x)
    x = tf.keras.layers.BatchNormalization()(x)
...
# 공통 평가 및 시각화 함수
def evaluate_and_visualize(model_to_evaluate, name_prefix="test2_v2"):
    # 테스트 데이터로 모델 평가
    print("\n최종 모델 평가...")
    test_loss, test_accuracy = model_to_evaluate.evaluate(test_dataset)
    print(f"테스트 정확도: {test_accuracy:.4f}")

    # 전체 테스트 데이터에 대한 예측
    print("\n클래스별 정확도 계산 중...")
    predictions = model_to_evaluate.predict(test_dataset)
    ...
    # 모델 저장
    model_to_evaluate.save(f'{name_prefix}_cnn_model.h5')
    print(f"모델이 {name_prefix}_cnn_model.h5로 저장되었습니다.")

# 최적 모델 로드 및 평가
try:
    # 최적 모델 로드
    best_model = tf.keras.models.load_model('best_model_test2_v2.h5')
    evaluate_and_visualize(best_model, "test2_v2")
except Exception as e:
    print(f"최적 모델 로드 중 오류 발생: {str(e)}")
    print("현재 모델 사용...")
    evaluate_and_visualize(model, "test2_v2")
