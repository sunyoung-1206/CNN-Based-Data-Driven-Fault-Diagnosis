...
# 공통 평가 및 시각화 함수
def evaluate_and_visualize(model_to_evaluate, name_prefix="test2"):
    # 테스트 데이터로 모델 평가
    print("\n최종 모델 평가...")
...
try:
    # 최적 모델 로드
    best_model = tf.keras.models.load_model('best_model_test2.h5')
    evaluate_and_visualize(best_model, "test2")
except Exception as e:
    print(f"최적 모델 로드 중 오류 발생: {str(e)}")
    print("현재 모델 사용...")
    evaluate_and_visualize(model, "test2")
